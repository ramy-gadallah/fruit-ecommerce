...
@section('title')
    {{ config()->get('app.name') }} | {{ trns('blogs') }}
@endsection
@section('page_name') {{ trns('new_letter') }} @endsection
@section('content')
        ...
        <div class="col-md-12 col-lg-12">
            <div class="card">
                <div class="card-header">
                    <h3 class="card-title"> {{ trns('new_letter') }} {{ config()->get('app.name') }}</h3>
                    <div class="">
                    </div>
                </div>
                <div class="card-body">
...
@section('ajaxCalls')
    <script>
        ...
        showData('{{route('newLetter.index')}}', columns);
        // Delete Using Ajax
        deleteScript('{{route('newLetter.destroy',':id')}}');
        // Add Using Ajax
        showAddModal('{{route('newLetter.create')}}');
        addScript();
        // Add Using Ajax
        showEditModal('{{route('newLetter.edit',':id')}}');
        editScript();
    </script>
@endsection
