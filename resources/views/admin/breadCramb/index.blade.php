...
@section('title')
    {{ config()->get('app.name') }} | {{ trns('breadCramb') }}
@endsection
@section('page_name') {{ trns('breadCramb') }} @endsection
@section('content')
        ...
        <div class="col-md-12 col-lg-12">
            <div class="card">
                <div class="card-header">
                    <h3 class="card-title"> {{ trns('breadCramb') }} {{ config()->get('app.name') }}</h3>
                    <div class="">
                    ...
                    <div class="table-responsive">
                        ...
                        <table class="table table-bordered text-nowrap w-100" id="dataTable">
                            ...
                            <tr class="fw-bolder text-muted bg-light">
                                ...
                                <th class="min-w-50px">{{ trns('page') }}</th>
...
@section('ajaxCalls')
    <script>
        ...
        showData('{{route('breadcrumb.index')}}', columns);
        // Delete Using Ajax
        deleteScript('{{route('breadcrumb.destroy',':id')}}');
        // Add Using Ajax
        showAddModal('{{route('breadcrumb.create')}}');
        addScript();
        // Add Using Ajax
        showEditModal('{{route('breadcrumb.edit',':id')}}');
        editScript();
    </script>
@endsection
@section('js')
<script>
    ...
    $(document).on('click', '.statusBtn', function() {
        ...
        $.ajax({
            url: '{{ route('changebreadcrumbStatus') }}',
            type: 'POST',
            data: {
                "_token": "{{ csrf_token() }}",
                'id': id,
