...
@section('title')
    {{ config()->get('app.name') }} | {{ trns('coupon') }}
@endsection
@section('page_name') {{ trns('coupon') }} @endsection
@section('content')
        ...
        <div class="col-md-12 col-lg-12">
            <div class="card">
                <div class="card-header">
                    <h3 class="card-title"> {{ trns('coupon') }} {{ config()->get('app.name') }}</h3>
                    <div class="">
                    ...
                    <div class="table-responsive">
                        ...
                        <table class="table table-bordered text-nowrap w-100" id="dataTable">
                            ...
                            <tr class="fw-bolder text-muted bg-light">
                                ...
                                <th class="min-w-50px">{{ trns('to') }}</th>
                                <th class="min-w-50px">{{ trns('discount') }}</th>
                                <th class="min-w-50px">{{ trns('count') }}</th>
...
@section('ajaxCalls')
    <script>
        ...
        // Delete Using Ajax
        deleteScript('{{route('coupon.destroy',':id')}}');
        // Add Using Ajax
        showAddModal('{{route('coupon.create')}}');
        addScript();
        // Add Using Ajax
        showEditModal('{{route('coupon.edit',':id')}}');
        editScript();
    </script>
@endsection
